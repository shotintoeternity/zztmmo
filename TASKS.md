...
11. **M18.17 — the next task (owner decision 2026-08-06).** A displaced socket is
   ...
   **M34.2 was filed 2026-08-10** now that there is a real ledger to write from:
   ...
   that a past day is written once. **M34.2 landed 2026-08-10** as specced:
...
- [x] **M17.11 — Show how many people are playing or editing each world (owner
  request 2026-07-20).** A player choosing a world now sees where other people
  actually are, and how busy the server is before they open the picker.

  Server. `WorldListEntry` carries `Editors` beside `Players`
  (`json:"editors,omitempty"`, so a quiet world stays absent from the JSON and
  the shape stays backward-compatible), populated by `handleWorlds` from
  `WebSocketServer.EditorCounts()`. That walks the session map under the server
  lock and asks each session for `MemberCount()` under its own lock; nothing
  outside `EditorSession` ever reads the mutex-guarded `Members` map.

  Picker. Each entry's occupancy is its own line beneath the byline — "(2
  players currently online, 3 editors)", or either half alone — clamped to the
  entry width, so it cannot collide with the title/author columns at any
  supported width (the M17.2 regression). Zeros print nothing, and
  `worldSearchHasOccupancy` keeps the selection highlight's line accounting in
  step with what is rendered.

  Title screen. `drawTitleSidebar` draws the server-wide total as " Playing:" /
  " Editing:" rows above the World row, sharing the menu's text column with the
  counts clear of the labels. A zero draws nothing rather than "0", and the rows
  collapse upward so a lone count never floats.

  Live, not a join-time snapshot. `refreshOccupancy` re-reads `/api/worlds` —
  the path the picker already uses, so no new route — every five seconds while
  the title screen is up, which is also the whole time the picker can be open.
  It updates the title total and rewrites the counts of the entries on screen in
  place (`applyWorldOccupancy`), leaving titles, authors, and Museum provenance
  alone. Polling stops on entering play or the editor, where the sidebar belongs
  to the game.

  Determinism: occupancy is server-observed presentation state. It never enters
  the simulation, a replay, or the parity oracle, and no fixture or hash was
  touched. No parity manifest row was added: no route is new, and the manifest's
  dimensions are routes and surfaces rather than JSON fields — `route.api.worlds`,
  `mode.modal-picker`, and `service.world-picker` already cover these surfaces.

  Verified by `TestM1711WorldsAPIReportsLiveEditingOccupancy` (playing and
  editing occupancy reported separately per world through `/api/worlds`, falling
  as people leave, with the counts read concurrently against a churning editor
  session — clean under `go test -race`), `TestWorldListEntriesReportEditors`
  (the entry-level contract including omitempty), and web tests covering the
  picker's editors-only and combined lines with their line accounting, the
  title-screen totals (stacking, collapse, no overprinting, nothing past column
  79), and `applyWorldOccupancy`/`worldOccupancyTotal`. The multi-browser
...
- [x] **M16.2 [ADVISOR] — Establish an independent vanilla oracle and scenario
  ...
  Landed: the oracle is the **real ZZT.EXE v3.2** (Museum of ZZT `zzt.zip`,
  sha256-pinned) under a pinned Zeta build with a custom headless frontend
  (`oracle/frontend_oracle.c`) — virtual clock, scripted keys, checkpoints read
  from emulated VRAM plus PC-speaker transitions. `make oracle-regen` is the
  explicit maintainer command; tests compare offline against the committed
  `fixtures/oracle/` captures (micro-world `ORCLROOM`, scenarios, captures,
  `provenance.json` with every pin). `engine/oracle_parity_test.go` replays the
  shared `.scn` scripts and compares cells, sidebar-parsed counters, modal
  scroll content, and tone-onset sequences, with three documented
  normalizations (pause blink, modal scrolls→ScrollEvent, the stubbed walk
  click); two perturbation tests prove fail-closed with the exact cell/counter
  named. The seam already paid for itself: it pinned vanilla's real cycle
  cadence (TickTimeDuration is *hundredths of a second* — ~2 PIT ticks per
  cycle at speed 4, not 8) and caught `SoundInitFreqTable` truncating C notes
  to 511/255 Hz where Turbo Pascal's 48-bit real plays 512/256 (fixed in
  sounds.go; presentation-only, replay fixture unchanged). Findings for later
  sweeps in NOTES.md: vanilla's walk click (`Sound(110)`) is stubbed in the
  port (M16.6), and a passage-paused player standing on a non-player tile can
  never unpause in the headless step loop (M16.3). RNG: the compared path is
  RNG-free by scenario design; VRAM is the oracle's exposed surface — tiles,
  stats, and RandSeed are compared via their screen projection, not memory
...
- [x] **M34.2 — the edition: the day's ledger written up, in a register the
  ...
  **Landed 2026-08-10.** `GazetteEditor` (engine/gazette_edition.go) holds the
  cache, the spend rules and the parser; `/api/gazette/edition` serves it and
  kicks the single-flight refresh; the ledger grew `dayEntries` so the naming
  reader and the tokenizing writer cannot disagree about what a day looked like.
  The width guarantee ended up deliberately split across two places, which is
  worth saying because it reads like a duplicate: the parser refuses a raw line
  wider than the 41 columns the prompt asks for, and the renderer re-wraps
  through `wrapZWDText` AFTER substitution, because a token is four characters
  and a display name is up to twenty-five — only the second half can be the
  guarantee, and the reserved 42nd column is what lets the OOP-markup guard
  prefix its space without overflowing. Three claims verified by mutation:
  accepting an unknown actor token reddens the parse table and the
  does-not-poison-the-cache test, dropping the markup guard reddens the overflow
  test with `"@bogart took the gate."`, and dropping the fingerprint gate turns
  one author call into three. The only line changed outside the Gazette's own
  files is `callBlocks` delegating to a new `callBlocksWithTokens`, which is how
  an edition stops billing a board's ceiling; `cmd/` is untouched, because the
  editor builds itself on first use from the ledger's own directory and whatever
  author the environment already offers. It filed nothing. M34.3 — the board
  itself, posted in the lobby — stays deliberately unspecced for the third time
  and for the same reason each time: it now has a real edition to be written
