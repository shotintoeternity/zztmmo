...
To run the terminal game locally: install Go, clone the repo, and run `go run ./cmd/zztgo`. To smoke-test the headless engine without opening a terminal UI, run `go run ./cmd/zzt-smoke`. To start the JSON WebSocket server, run `go run ./cmd/zzt-server`. If you want the terminal game to look a bit more authentic, install an [IBM EGA font](https://int10h.org/oldschool-pc-fonts/fontlist/#ibmega) and adjust the line spacing to zero. On macOS you can use [this Terminal settings file](https://github.com/benhoyt/zztgo/blob/master/zzt.terminal).
