...
// The 1.75-glyph face rule (owner report, 2026-09-08). A tile is 1 wide and
// 1.75 deep, so a block's east and west faces cannot show a glyph exactly once.
// Only ZZT's four dither characters may repeat to fill them; everything else is
// a symbol and is drawn once, centred. A door repeated is one door and three
// quarters of a second door, on one square.
for (const [ch, want, why] of [
  [0xb0, true, "light shade is texture"],
  [0xb1, true, "the breakable wall's dither is texture"],
  [0xb2, true, "the normal wall's dither is texture"],
  [0xdb, true, "the solid block is texture"],
  [0x0a, false, "a door is a symbol"],
  [0xf0, false, "a passage is a symbol"],
  [0xfe, false, "a boulder is a symbol"],
  [0xba, false, "a line-drawing wall piece is a symbol"],
]) {
  assert.equal(classify(ch, 0x0f).tiles, want, `glyph 0x${ch.toString(16)}: ${why}`);
}
console.log("classify tiles ok");
